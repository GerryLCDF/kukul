...
function magneticCode(s: ControlSettings): string {
  ...
  return [
    ...
    '  const dx = e.clientX - (rect.left + rect.width / 2)',
    '  const dy = e.clientY - (rect.top + rect.height / 2)',
    `  x.set(dx * ${intensity}) // intensidad del imán`,
    `  y.set(dy * ${intensity})`,
    '}',
    ...
    '  onMouseLeave={onLeave}',
    tilt
      ? `  style={{ x, y, background: '${flat ? colorPrimary : `linear-gradient(135deg, ${colorPrimary}, ${colorAccent})`}', rotateX: glowY, rotateY: glowX }} // 3D activo`
      : `  style={{ x, y, background: '${flat ? colorPrimary : `linear-gradient(135deg, ${colorPrimary}, ${colorAccent})`}' }}`,
    noShadow ? '  // sin sombra' : '  whileHover={{ boxShadow: "0 14px 34px rgba(0,0,0,0.35)" }}',
    '  whileTap={{ scale: 0.92 }}',
    '>',
    '  Botón magnético',
    '</motion.button>',
