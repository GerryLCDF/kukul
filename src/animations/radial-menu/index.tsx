...
function radialCode(s: ControlSettings): string {
  ...
  const center = String(s.centerColor) || '#216e4b'
  const count = Math.max(1, Math.min(6, Number(s.count) || 6))
  const visible = items.slice(0, count)
  const imports = visible.map((it) => it.iconName).join(', ')
  const solidBg = solid ? 'background: it.color,' : 'background: `linear-gradient(135deg, ${it.color}, ${it.color}99)`,';
  ...
  return [
    ...
    `const ICON = 52 // diámetro de cada opción`,
    '',
    `const ITEMS = [`,
    ...visible.map((it) => `  { label: '${it.label}', Icon: ${it.iconName}, color: '${it.color}' },`),
    `]`,
    '',
    '// posición circular de cada opción',
    'function itemPos(index: number, count: number) {',
    '  const a = (index / count) * Math.PI * 2 - Math.PI / 2',
    '  return { x: Math.cos(a) * R, y: Math.sin(a) * R }',
    '}',
    ...
    '        {open &&',
    `          ITEMS.map((it, i) => {`,
    '            const p = itemPos(i, ITEMS.length)',
    '            const active = hover === i',
    '            const dimmed = hover !== null && hover !== i',
    '            const lbl = { x: Math.cos((i / ITEMS.length) * Math.PI * 2 - Math.PI / 2) * (ICON / 2 + 4), y: Math.sin((i / ITEMS.length) * Math.PI * 2 - Math.PI / 2) * (ICON / 2 + 4) }',
    '            return (',
    '              <motion.div',
    '                key={it.label}',
    '                initial={{ scale: 0, opacity: 0, x: 0, y: (i === 0 ? -1 : 1) * 14 }}',
    `                animate={{ scale: 1, opacity: 1, x: p.x, y: p.y, transition: { ...spring, delay: 0.12 + i * 0.06 } }}`,
    '                exit={{ scale: 0, opacity: 0, transition: { duration: 0.16 } }}',
    '                onMouseEnter={() => setHover(i)}',
    '                onMouseLeave={() => setHover((h) => (h === i ? null : h))}',
    "                style={{ position: 'absolute', left: '50%', top: '50%', margin: -ICON / 2, cursor: 'pointer', zIndex: 10 }}",
    '              >',
    '                <motion.div',
    '                  animate={{',
    '                    scale: active ? 1.28 : dimmed ? 0.82 : 1,',
    '                    opacity: dimmed ? 0.55 : 1,',
    wobble
      ? '                    rotate: active ? [0, -11, 11, -6, 0] : 0,'
      : '                    rotate: 0,',
    glow
      ? '                    boxShadow: active ? `0 0 26px ${it.color}99` : `0 6px 16px ${it.color}44`,'
      : noShadow
        ? '                    boxShadow: "none",'
    ...
    '                  }}',
    '                  style={{',
    `                    width: ${'ICON'}, height: ${'ICON'}, borderRadius: 999,`,
    `                    ${solidBg}`,
    '                    display: "grid",',
    '                    placeItems: "center",',
    '                    border: "2px solid rgba(255,255,255,0.15)",',
    ...
    '                        position: "absolute",',
    '                        left: "50%", top: "50%",',
    '                        transform: `translate(-50%, -50%) translate(${lbl.x}px, ${lbl.y}px)`,',
    '                        background: "#1b1b1b",',
