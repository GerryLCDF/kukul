...
function knobCode(s: ControlSettings): string {
  ...
  return [
    ...
    '  let t = ((raw - MIN_ANG) % 360 + 360) % 360',
    '  if (t > SWEEP) t = SWEEP',
    `  ${stepped ? 'return Math.round(t / SWEEP / STEP_FRAC) * STEP_FRAC' : 'return t / SWEEP'}`,
    '}',
    '',
    ...
    '  setValue(angleToFrac(raw) * 100)',
    '}',
    '',
    `// estado desactivado: escala de grises y sin puntero`,
    `<div role="slider" aria-disabled={${disabled}}`,
    `  onPointerDown={${disabled ? 'undefined' : 'onPointerDown'}}`,
    '  style={{',
    `    border: '2px solid ${disabled ? '#3a3a3a' : alpha(colorPrimary, 0.6)}',`,
    `    background: '${disabled ? '#1e1e1e' : '#1b1b1b'}',`,
    disabled
      ? '    // gris @disabled'
      : glow
        ? `    boxShadow: '0 6px 18px ${alpha(colorPrimary, 0.25)}', // brillo`
        : noShadow
    ...
    '  {/* centro: hub */}',
    flat
      ? `  <div style={{ background: '${colorPrimary}' }} />` // color plano
      : `  <div style={{ background: 'linear-gradient(135deg, ${colorPrimary}, ${colorAccent})' }} />`,
    `  {/* ${stepped ? 'líneas de pasos alrededor del círculo' : 'línea fluida'} */}`,
    `  <div style={{ transform: 'rotate(' + (frac * SWEEP - 135) + 'deg)', background: '${disabled ? '#555' : colorAccent}'${stepped ? ', // solo línea en modo fluido' : ''} }} />`,
    '</div>',
  ].join('\n')
}
