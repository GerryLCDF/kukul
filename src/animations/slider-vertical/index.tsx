...
function sliderCode(s: ControlSettings): string {
  ...
  return [
    'const H = 220',
    'const PAD = 22 // reserva para los extremos',
    `const STEP_FRAC = ${stepped ? `1 / (${steps} - 1)` : '1'}${stepped ? '' : ' // sin pasos'}`,
    '',
    'function yToFrac(clientY, rect) {',
    '  const inner = rect.height - PAD * 2',
    '  const f = (rect.bottom - PAD - clientY) / inner // 0 abajo, 1 arriba',
    `  const clamped = Math.max(0, Math.min(1, f))`,
    `  ${stepped ? 'return Math.round(clamped / STEP_FRAC) * STEP_FRAC' : 'return clamped'}`,
    '}',
    '',
    'const onPointerDown = (e) => {',
    '  const rect = track.getBoundingClientRect()',
    '  setValue(yToFrac(e.clientY, rect) * 100)',
    '}',
    '',
    `// estado desactivado: gris y sin interacción`,
    `<div role="slider" aria-orientation="vertical" aria-disabled={${disabled}}`,
    `  onPointerDown={${disabled ? 'undefined' : 'onPointerDown'}}`,
    '  style={{',
    `    background: '${disabled ? '#2c2c2c' : alpha(colorPrimary, 0.18)}',`,
disabled
      ? '    // gris @disabled'
      : glow
        ? `    boxShadow: '0 6px 18px ${alpha(colorPrimary, 0.22)}', // brillo`
        : noShadow
          ? '    boxShadow: "none", // sin sombra'
          : '    boxShadow: "0 4px 8px rgba(0,0,0,0.4)", // sin brillo',
    '  }}',
    '>',
    '  {/* relleno: height = frac * innerH */}',
    flat
      ? `  <div style={{ height: frac * (H - PAD * 2), background: '${disabled ? '#3a3a3a' : colorPrimary}' }} />`
      : `  <div style={{ height: frac * (H - PAD * 2), background: '${disabled ? '#3a3a3a' : `linear-gradient(180deg, ${colorAccent}, ${colorPrimary})`}' }} />`,
    '  {/* pomo: top = PAD + (1 - frac) * innerH */}',
    '</div>',
  ].join('\n')
}
